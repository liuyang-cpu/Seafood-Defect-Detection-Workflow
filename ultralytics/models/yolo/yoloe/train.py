...
class YOLOEPEFreeTrainer(YOLOEPETrainer, YOLOETrainerFromScratch):
    """Train prompts-free YOLOE model.

    This trainer combines linear probing capabilities with from-scratch training for prompts-free YOLOE models that
    don't require text prompts during inference.

    Methods:
        get_validator: Return standard DetectionValidator for validation.
        preprocess_batch: Preprocess batches without text features.
        set_text_embeddings: Set text embeddings for datasets (no-op for prompts-free).
    """
...
class YOLOEVPTrainer(YOLOETrainerFromScratch):
    ...
    This trainer extends YOLOETrainerFromScratch to support visual prompts-based training, where visual cues are
    provided alongside images to guide the detection process.

    Methods:
        build_dataset: Build dataset with visual prompts loading transforms.
    ...
    def _close_dataloader_mosaic(self):
        """Close mosaic augmentation and add visual prompts loading to the training dataset."""
