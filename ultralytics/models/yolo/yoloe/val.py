...
class YOLOEDetectValidator(DetectionValidator):
    """
    A validator class for YOLOE detection models that handles both text and visual prompts embeddings.

    This class extends DetectionValidator to provide specialized validation functionality for YOLOE models.
    It supports validation using either text prompts or visual prompts embeddings extracted from training samples,
    enabling flexible evaluation strategies for prompts-based object detection.
    ...
    Methods:
        get_visual_pe: Extract visual prompts embeddings from training samples.
        preprocess: Preprocess batch data ensuring visuals are on the same device as images.
        get_vpe_dataloader: Create a dataloader for LVIS training visual prompts samples.
        __call__: Run validation using either text or visual prompts embeddings.
    ...
    def get_visual_pe(self, dataloader: torch.utils.data.DataLoader, model: YOLOEModel) -> torch.Tensor:
        """
        Extract visual prompts embeddings from training samples.

        This method processes a dataloader to compute visual prompts embeddings for each class using a YOLOE model.
        ...
        Args:
            ...
            model (YOLOEModel): The YOLOE model from which to extract visual prompts embeddings.

        Returns:
            (torch.Tensor): Visual prompts embeddings with shape (1, num_classes, embed_dim).
        """
        ...
        desc = "Get visual prompts embeddings from samples"
        ...
        # Extract visual prompts embeddings
    ...
    def get_vpe_dataloader(self, data: Dict[str, Any]) -> torch.utils.data.DataLoader:
        """
        Create a dataloader for LVIS training visual prompts samples.

        This method prepares a dataloader for visual prompts embeddings (VPE) using the specified dataset.
        ...
        Returns:
            (torch.utils.data.DataLoader): The dataloader for visual prompts samples.
        """
    ...
    def __call__(
        ...
        """
        Run validation on the model using either text or visual prompts embeddings.
        ...
        if trainer is not None:
            ...
            if load_vp:
                LOGGER.info("Validate using the visual prompts.")
                ...
                LOGGER.info("Validate using the text prompts.")
                tpe = model.get_text_pe(names)
                model.set_classes(names, tpe)
            stats = super().__call__(trainer, model)
        else:
            if refer_data is not None:
                assert load_vp, "Refer data is only used for visual prompts validation."
            ...
            if load_vp:
                ...
                # could use same dataset or refer to extract visual prompts embeddings
                dataloader = self.get_vpe_dataloader(data)
                vpe = self.get_visual_pe(dataloader, model)
                model.set_classes(names, vpe)
                stats = super().__call__(model=deepcopy(model))
            elif isinstance(model.model[-1], YOLOEDetect) and hasattr(model.model[-1], "lrpc"):  # prompts-free
...
class YOLOESegValidator(YOLOEDetectValidator, SegmentationValidator):
    """YOLOE segmentation validator that supports both text and visual prompts embeddings."""
