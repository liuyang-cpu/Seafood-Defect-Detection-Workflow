...
class YOLOEDetect(Detect):
    ...
    through text embeddings and visual prompts embeddings.

    Attributes:
        is_fused (bool): Whether the model is fused for inference.
        cv3 (nn.ModuleList): Convolution layers for embedding features.
        cv4 (nn.ModuleList): Contrastive head layers for text-vision alignment.
        reprta (Residual): Residual block for text prompts embeddings.
        savpe (SAVPE): Spatial-aware visual prompts embeddings module.
        embed (int): Embedding dimension.

    Methods:
        fuse: Fuse text features with model weights for efficient inference.
        get_tpe: Get text prompts embeddings with normalization.
        get_vpe: Get visual prompts embeddings with spatial awareness.
        forward_lrpc: Process features with fused text embeddings for prompts-free model.
        forward: Process features with class prompts embeddings to generate detections.
    ...
    def get_tpe(self, tpe: torch.Tensor | None) -> torch.Tensor | None:
        """Get text prompts embeddings with normalization."""
        return None if tpe is None else F.normalize(self.reprta(tpe), dim=-1, p=2)

    def get_vpe(self, x: list[torch.Tensor], vpe: torch.Tensor) -> torch.Tensor:
        """Get visual prompts embeddings with spatial awareness."""
        if vpe.shape[1] == 0:  # no visual prompts embeddings
    ...
    def forward_lrpc(self, x: list[torch.Tensor], return_mask: bool = False) -> torch.Tensor | tuple:
        """Process features with fused text embeddings to generate detections for prompts-free model."""
        masks = []
    ...
    def forward(self, x: list[torch.Tensor], cls_pe: torch.Tensor, return_mask: bool = False) -> torch.Tensor | tuple:
        """Process features with class prompts embeddings to generate detections."""
        if hasattr(self, "lrpc"):  # for prompts-free inference
